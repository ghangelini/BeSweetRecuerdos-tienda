...
export default function AdminDashboard() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    const adminPassword = process.env.NEXT_PUBLIC_ADMIN_PASSWORD || 'Besweet2026';
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#fdfaf6] px-4">
        <div className="max-w-md w-full bg-white p-8 rounded-2xl shadow-xl border border-pink-100">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-pink-500 focus:border-pink-500 bg-gray-50 text-gray-900 outline-none transition-all pr-12"
                  placeholder="••••••••"
                  required
                />
                <button
            ...
            <button
              type="submit"
              className="w-full py-3 px-4 bg-pink-500 hover:bg-pink-600 text-white font-bold rounded-xl shadow-lg shadow-pink-200 transition-all transform hover:-translate-y-0.5"
            >
              Ingresar al Panel
            </button>
          </form>
          <div className="mt-6 text-xs text-center text-gray-400">
            BeSweet Recuerdos © {new Date().getFullYear()}
          </div>
        </div>
      </div>
    );
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          <div>
            ...
            <p className="text-gray-500 font-medium">Gestiona tu tienda y revisa tus pedidos</p>
          </div>
        ...
        {activeTab === 'orders' ? (
          <div>
            <div className="flex justify-between items-center mb-6">
              ...
              <button 
                onClick={fetchOrders}
                className="px-4 py-2 bg-white text-pink-600 border border-pink-200 font-semibold rounded-lg shadow-sm hover:bg-pink-50 transition-all flex items-center gap-2"
              >
